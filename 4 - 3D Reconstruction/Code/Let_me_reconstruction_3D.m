...
% comment appropriate lines (223-230 in my script - begins with load) and uncomment lines where we are selecting the points (95-210)
...
% We are using numPts_cube for rectangle as well - 8 vertices for both.
...
%Below code is for showing clicked points
while k <= numPts_book
[a,b] = ginput(1);
plot(a,b,'r+');
Px5(k)=a;
...
k=1;
%Below Line is code for showing the clicked points
while k <= numPts_book
[a,b] = ginput(1);
plot(a,b,'r+');
Px6(k)=a;
...
figure(3)
for cpt=1:numPts_py
    plot3(PW_pyr(1,cpt),PW_pyr(2,cpt),PW_pyr(3,cpt),'rd'); hold on; % Draw point
end
...
% figure(4)
for cpt=1:numPts_cube
    plot3(PW_cube(1,cpt),PW_cube(2,cpt),PW_cube(3,cpt),'mo'); hold on; % Draw point
end
...
% figure(4)
for cpt=1:numPts_book
    plot3(PW_book(1,cpt),PW_book(2,cpt),PW_book(3,cpt),'r*'); hold on; % Draw point
end
